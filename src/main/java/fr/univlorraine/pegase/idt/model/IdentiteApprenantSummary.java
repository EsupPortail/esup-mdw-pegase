...
public class IdentiteApprenantSummary {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(id, identifiantApprenantPegase, nomNaissance, nomUsage, prenom, prenom2, prenom3, email, sexe, codeApprenant, ineMaitre, inesSupplementaires, dateNaissance, communeNaissance, paysNaissance, nationalite, actif, temoinDoublonPotentiel, temoinDecede);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IdentiteApprenantSummary.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in IdentiteApprenantSummary is not found in the empty JSON string", IdentiteApprenantSummary.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!IdentiteApprenantSummary.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `IdentiteApprenantSummary` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
