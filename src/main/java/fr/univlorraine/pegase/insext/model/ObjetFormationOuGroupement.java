...
public class ObjetFormationOuGroupement extends ObjetAvecLibelle {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("est1", "code", "libelleCourt", "libelleLong", "nature"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("nature", "est1"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ObjetFormationOuGroupement.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in ObjetFormationOuGroupement is not found in the empty JSON string", ObjetFormationOuGroupement.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ObjetFormationOuGroupement.openapiFields.contains(entry.getKey())) {
          //throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ObjetFormationOuGroupement` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
          return;
        }
      }
