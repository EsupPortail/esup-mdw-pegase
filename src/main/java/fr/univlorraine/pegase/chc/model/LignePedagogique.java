...
public class LignePedagogique {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!LignePedagogique.openapiFields.contains(entry.getKey())) {
          //throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `LignePedagogique` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
          return;
        }
      }
