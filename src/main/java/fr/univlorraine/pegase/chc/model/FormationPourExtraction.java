...
public class FormationPourExtraction {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!FormationPourExtraction.openapiFields.contains(entry.getKey())) {
          //throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `FormationPourExtraction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
          return;
        }
      }
