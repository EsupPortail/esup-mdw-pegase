...
/**
 * Un type de groupe de la composition
 */
...
public class TypeGroupe {
  ...
  public TypeGroupe code(@jakarta.annotation.Nonnull String code) {
    this.code = code;
    return this;
  }
  ...
  public TypeGroupe libelleCourt(@jakarta.annotation.Nullable String libelleCourt) {
    this.libelleCourt = libelleCourt;
    return this;
  }

  /**
   * Le libellé court du type de groupe
   * @return libelleCourt
   */
  @jakarta.annotation.Nullable
  ...
  public TypeGroupe libelleLong(@jakarta.annotation.Nullable String libelleLong) {
    this.libelleLong = libelleLong;
    return this;
  }

  /**
   * Le libellé long du type de groupe
   * @return libelleLong
   */
  @jakarta.annotation.Nullable
  ...
  public boolean equals(Object o) {
    ...
    TypeGroupe typeGroupe = (TypeGroupe) o;
    return Objects.equals(this.code, typeGroupe.code) &&
        Objects.equals(this.libelleCourt, typeGroupe.libelleCourt) &&
        Objects.equals(this.libelleLong, typeGroupe.libelleLong);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to TypeGroupe
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TypeGroupe.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in TypeGroupe is not found in the empty JSON string", TypeGroupe.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TypeGroupe.openapiFields.contains(entry.getKey())) {
          //throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `TypeGroupe` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
          return;
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : TypeGroupe.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TypeGroupe.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TypeGroupe' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TypeGroupe> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TypeGroupe.class));

       return (TypeAdapter<T>) new TypeAdapter<TypeGroupe>() {
           @Override
           public void write(JsonWriter out, TypeGroupe value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TypeGroupe read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TypeGroupe given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TypeGroupe
   * @throws IOException if the JSON string is invalid with respect to TypeGroupe
   */
  public static TypeGroupe fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TypeGroupe.class);
  }

  /**
   * Convert an instance of TypeGroupe to an JSON string
   *
   * @return JSON string
   */
